...
<head>
  ...
  <script>
    ...
    async function loadList(page = 1) {
      ...
      try {
        const res = await fetch(`api/getMailList?page=${page}&size=${pageSize}`);
    ...
    async function loadEmail(id) {
      ...
      try {
        const res = await fetch('api/getMailInfo?id=' + encodeURIComponent(id));
        if (!res.ok) throw new Error('HTTP error! status: ' + res.status);
...
<body>
  <div class="container">
    ...
    <div class="controls">
      <button id="refresh-btn" class="btn btn-primary" onclick="loadList(currentPage)">Reload</button>
      <a id="compose-link" href="compose.jsp" class="btn btn-secondary">Compose</a>
      <div class="pagination">
        <button id="prev-btn" class="btn btn-secondary" disabled onclick="subPage(); loadList(currentPage)">&laquo; Previous</button>
        <span class="page-info" id="page-info">1/1</span>
        <button id="next-btn" class="btn btn-secondary" disabled onclick="addPage(); loadList(currentPage)">Next &raquo;</button>

        <div class="page-jump">
          <label id="jump-label" for="page-input">Page</label>
          <input type="number" id="page-input" min="1" placeholder="1">
          <button id="go-btn" class="btn btn-secondary">get</button>
        </div>
      </div>
    </div>
